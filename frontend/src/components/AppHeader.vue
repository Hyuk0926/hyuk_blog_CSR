<template>
  <header class="navbar">
    <div class="container header-flex">
      <!-- 로고/브랜드 - 기존 스타일 유지하면서 Tailwind 보조 적용 -->
      <router-link 
        :to="homeLink" 
        class="navbar-brand"
      >
        {{ $i18n.locale === 'ja' ? '高原のブログ' : 'Takahara\'s Blog' }}
      </router-link>
      ...
      <div class="header-right">
        ...
        <nav>
          <ul class="navbar-nav">
            <li class="nav-item">
              <router-link :to="getLink('/knowledge')" class="nav-link">
                {{ $t('nav.knowledge') }}
              </router-link>
            </li>
            <li class="nav-item">
              <router-link :to="getLink('/projects')" class="nav-link">
                {{ $t('nav.projects') }}
              </router-link>
            </li>
            <li class="nav-item">
              <router-link :to="getLink('/resume')" class="nav-link">
                {{ $t('nav.resume') }}
              </router-link>
            </li>
            <li class="nav-item">
              <router-link :to="getLink('/contact')" class="nav-link">
                {{ $t('nav.contact') }}
              </router-link>
            </li>
          </ul>
        ...
        <!-- 우측 컨트롤 - 기존 구조 유지하면서 Tailwind 개선 -->
        <div class="header-controls">
          ...
          <div v-else class="auth-controls">
            <router-link to="/user/login" class="auth-btn login-btn">
              {{ $t('nav.login') }}
            </router-link>
            <router-link to="/user/register" class="auth-btn register-btn">
              {{ $t('nav.register') }}
            </router-link>
          </div>
          ...
          <div id="langToggle" class="lang-toggle" :class="{ disabled: isLanguageChanging }">
            <button @click="setLang('ko')" class="lang-btn" :class="{ active: $i18n.locale === 'ko' }">KO</button>
            <span class="lang-separator">|</span>
            <button @click="setLang('ja')" class="lang-btn" :class="{ active: $i18n.locale === 'ja' }">JP</button>
          </div>
        </div>
      </div>
    </div>
...
<style scoped>
/* 기존 헤더 스타일 복원 + Tailwind CSS 보조 적용 */
.navbar { 
  background: #ffffff; 
